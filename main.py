...
tmp5=[documentsq[i] for i in range(len(documentsq)) if i in tmp3]
tmp6=[documentsq[i] for i in range(len(documentsq)) if i in tmp4]
...
with open('result/new_add_check.txt','w') as f:
    ...
    for i in tmp3:
        xiangsiwenben=documentsb[tmp[i]] #跟i最相似的文本是
        wenben=documentsq[i]
        f.write(i)
